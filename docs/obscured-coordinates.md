...
**271 specimens from 57 observations hold a stand-in, and all 271 are on printed labels**, dated between 31 December 2019 and 13 May 2026. One example: field number OBA_1907841, printed 31 December 2019, label reads "Jefferson, 44.7830 -123.1650", which is 11.6 km from the collection site and inside its 0.2° cell: an earlier draw. Another, OBA_2125556 ("Christmas Valley, 43.3780 -120.2870"), was iNaturalist's current stand-in on 22 September and an older one on 23 September; the observation was edited in between and iNaturalist redrew, which is the mechanism in action. The older stand-ins miss the true point by about 11 km on average, and they sit in the same 0.2° cell as the true point, which is what a previous random draw from the cell looks like and what a moved pin does not. Stored pairs are rounded to four decimals, so one can land exactly on a cell edge; the script allows for that.
...
The measurement is [`scripts/audit-obscured-coordinates.mjs`](../scripts/audit-obscured-coordinates.mjs). It runs inside the worker container, reads nothing but the API and the occurrences collection, prints the tables above as JSON, and writes two lists (field number, print date, collection year, collector, atlas, what the label says, distance, observation): the stand-in specimens to `audit-wrong-points.csv`, for whoever decides about the pins, and the unsettled ones to `audit-unsettled-points.csv`, for whoever asks their observers for access. Run it again whenever the number matters, in particular on the day the cutover dump is taken: the count drifts down as observers grant access and a task selects their records, and it never goes up.

### Labels already printed

What to do about the 271 pinned labels is Arthur's decision, filed as [#60](https://github.com/oregon-bee-project/OBP-Server/issues/60) with breakdowns by collection year and atlas. The 44 that cannot be settled yet, 40 for want of access and 4 that sit between 100 m and 5 km from the true point, are [#61](https://github.com/oregon-bee-project/OBP-Server/issues/61), by year and collector. The two options: correct the 271 records, leaving those pins deliberately wrong and the record right; or leave the records matching their pins. Beeline is built to hold only coordinates it believes true, so the decision is about the pins, not about the new system. His earlier suggestion of a note on each changed record, and his answers about a CSV column, were given before we knew the population; the column will not be added here, and the note belongs in Beeline if anywhere.
...
| --- | --- | --- |
| The 271 pinned labels: correct the records or keep them matching the pins | Arthur | [#60](https://github.com/oregon-bee-project/OBP-Server/issues/60) |
| The 44 labels nobody can check: two collectors to grant access, one to confirm a pin | Arthur, via the collectors | [#61](https://github.com/oregon-bee-project/OBP-Server/issues/61) |
...
| Correct the three Beeline comments that call production provenance-free | Peter | Beeline |
